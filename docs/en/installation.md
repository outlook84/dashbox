...
dashbox --data-dir data --host 0.0.0.0 --port 18990
```
...
To pick up faster pre-release yt-dlp fixes:
...
```bash
git clone https://github.com/solidove/dashbox.git
cd dashbox
```

Install dependencies, build the bundled TVBox spider and admin UI, then run the
service:
...
uv run dashbox --data-dir data --host 0.0.0.0 --port 18990
```
...
```bash
dashbox -c config.json --host 0.0.0.0 --port 18990
```

Runtime settings such as `--public-base-url`, `DASHBOX_UPSTREAM_TIMEOUT`, and
`DASHBOX_UNSAFE_IMAGE_PROXY_MODE` are documented in
[Configuration fields](config-fields.md).
...
The same value can be supplied with `DASHBOX_PUBLIC_BASE_URL`. Dashbox uses it
when generating TVBox spider URLs, media helper URLs, Kodi repository metadata,
and other self-referencing links.
